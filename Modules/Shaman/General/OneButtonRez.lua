...
local function BuildRezMacro(partyUnit)
    local lines = {}
    if partyUnit then
        lines[#lines + 1] = "/use [@" .. partyUnit .. ",help,dead] Ancestral Spirit"
    end
    lines[#lines + 1] = "/use [@mouseover,help,dead] Ancestral Spirit"
    lines[#lines + 1] = "/use [@target,help,dead] Ancestral Spirit"
    return table.concat(lines, "\n")
end

local secureBaseBtn = CreateHiddenSecureMacroButton("MyScriptsShamanRezBaseButton", BuildRezMacro(nil))
local secureShiftBtn = CreateHiddenSecureMacroButton("MyScriptsShamanRezShiftButton", BuildRezMacro("party1"))
local secureCtrlBtn = CreateHiddenSecureMacroButton("MyScriptsShamanRezCtrlButton", BuildRezMacro("party2"))
local secureShiftCtrlBtn = CreateHiddenSecureMacroButton("MyScriptsShamanRezShiftCtrlButton", BuildRezMacro("party3"))
local secureShiftAltBtn = CreateHiddenSecureMacroButton("MyScriptsShamanRezShiftAltButton", BuildRezMacro("party4"))
local secureMassBtn = CreateFrame("Button", "MyScriptsShamanMassRezButton", UIParent, "SecureActionButtonTemplate")
...
secureMassBtn:SetAttribute("spell", "Ancestral Vision")

local TRACKED_SPELL_NAMES = { "Ancestral Spirit", "Ancestral Vision" }
local TRACKED_MACRO_KEYWORDS = { "ancestral spirit", "ancestral vision", "rez", "res", "mass rez" }
...
local function SlotContainsTrackedRez(slot)
    ...
    if actionType == "macro" and actionID then
        ...
        if GetMacroInfo then
            local ok, _, _, macroBody = pcall(GetMacroInfo, actionID)
            if ok then
...
local function Rebind()
    ...
    local variantButtons = {
        BASE = "MyScriptsShamanRezBaseButton",
        ALT = "MyScriptsShamanMassRezButton",
        SHIFT = "MyScriptsShamanRezShiftButton",
        CTRL = "MyScriptsShamanRezCtrlButton",
        ["ALT+SHIFT"] = "MyScriptsShamanRezShiftAltButton",
        ["ALT+CTRL"] = "MyScriptsShamanRezBaseButton",
        ["CTRL+SHIFT"] = "MyScriptsShamanRezShiftCtrlButton",
        ["ALT+CTRL+SHIFT"] = "MyScriptsShamanRezBaseButton",
    }
